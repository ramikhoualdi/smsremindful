...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!code || !state) {
      return NextResponse.redirect(
        new URL(`${redirectTo}?error=missing_params`, request.url)
      )
    }

    // Verify state contains valid user info
    let stateData: { clerkId: string; redirectTo?: string }
    try {
      stateData = JSON.parse(Buffer.from(state, 'base64').toString())
      // Update redirectTo from state if provided
      if (stateData.redirectTo) {
        redirectTo = stateData.redirectTo
      }
    } catch {
      return NextResponse.redirect(
        new URL(`${redirectTo}?error=invalid_state`, request.url)
      )
    }
    ...
    if (!currentUserId || currentUserId !== stateData.clerkId) {
      return NextResponse.redirect(
        new URL(`${redirectTo}?error=user_mismatch`, request.url)
      )
    }
    ...
    try {
      ...
      return NextResponse.redirect(
        new URL(`${redirectTo}?error=token_exchange_failed`, request.url)
      )
    }
    ...
    if (!tokens.access_token || !tokens.refresh_token) {
      ...
      return NextResponse.redirect(
        new URL(`${redirectTo}?error=missing_tokens`, request.url)
      )
    }
    ...
    try {
      ...
      return NextResponse.redirect(
        new URL(`${redirectTo}?error=user_info_failed`, request.url)
      )
    }
    ...
    try {
      ...
      return NextResponse.redirect(
        new URL(`${redirectTo}?error=firestore_user_failed`, request.url)
      )
    }
    ...
    try {
      ...
      return NextResponse.redirect(
        new URL(`${redirectTo}?error=update_connection_failed`, request.url)
      )
    }

    // Redirect back to the original page with success
    return NextResponse.redirect(
      new URL(`${redirectTo}?success=calendar_connected`, request.url)
    )
  } catch (error) {
    console.error('Unexpected error in OAuth callback:', error)
    return NextResponse.redirect(
      new URL(`${redirectTo}?error=callback_failed`, request.url)
    )
  }
}
